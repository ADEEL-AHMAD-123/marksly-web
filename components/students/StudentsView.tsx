  ...
  return (
    <div className="space-y-6">
      ...
      {/* Help / reference — split into focused, independently-collapsible
          ...
          outrank the thing they came here to do. Kept as four separate
          notes deliberately (not consolidated into one) so an admin with a
          specific question in mind sees just that answer, not a longer note
          with three others they have to skim past first. */}
      ...
      <ImportCsvDrawer
        ...
        helpText={'Students never have their own email/phone — only guardian contact is collected. "guardianPhone" is required for every row; "guardianEmail" is optional (lets the guardian self-reset their PIN later, but isn\'t needed to create the account). "nationalIdNumber" (Form B/CNIC, format 42101-1234567-1) is also required for every row. "dateOfBirth" (e.g. 2015-04-12) is optional. Running more than one active term at once (e.g. overlapping semesters)? Add an optional "term" column with the exact term name if any class name exists in more than one active term — otherwise it can be left out.'}
        resultNote={
          <>
